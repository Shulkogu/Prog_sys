...
namespace Model
{
    internal class Saver
    {
        ...
        public Saver(Job Job, ref Logger Logger, ref ManualResetEvent ManualResetEvent)
        {
            this.Job = Job;
            ...
            this.Logger.CreateState(this.Job.Name, RelativeFilePaths.Length, TotalSize, Workstate.WAITING_TO_START);
        }
        ...
        //For all the paths within RelativeFilePaths, a File object will be created, the object will have the responsibility of saving the file or not. The time it took to treat the file is counted, and the logger is called.
        {
            ...
            {
                if(!this.ManualResetEvent.WaitOne(0))
                {
                    Logger.PauseState(this.Job.Name);
                    this.ManualResetEvent.WaitOne();
                    Logger.ResumeState(this.Job.Name);
                }
                float Time = DateTimeOffset.Now.ToUnixTimeMilliseconds();
                ...
                {
                    Logger.Log(this.Job.Name, this.Job.SourcePath + @"\\" + RelativePath, this.Job.TargetPath + @"\\" + FolderName + @"\\" + RelativePath, Workstate.ACTIVE, FileInfo.Item1.Value, Counter, DateTimeOffset.Now.ToUnixTimeMilliseconds() - Time, FileInfo.Item2);
                }
                else
                {
                    Logger.UpdateState(this.Job.Name, this.Job.SourcePath + @"\\" + RelativePath, this.Job.TargetPath + @"\\" + FolderName + @"\\" + RelativePath, Workstate.ACTIVE, Counter);
                }
                Counter++;
            }
